...
CMD_ID = f'{config.COMPANY_NAME}_{config.ADDIN_NAME}_ppModify'
CMD_NAME = 'Modify PP'
CMD_Description = 'Modify your PP'
PALETTE_NAME = 'PP Modifier'
IS_PROMOTED = True
...
def stop():
	...
	global _sketches, _components

	for sketch in _sketches:
		sketch.deleteMe()
...
def palette_incoming(html_args: adsk.core.HTMLEventArgs):
	...
	def format_msg(data):
		return "".join([f"<b>{key}</b>: {data[key]}<br/>" for key in data]) if data else "No data"
	...
	# 	msg += f'<b>Action</b>: {message_action}<br/><b>Data:<br/></b>{format_msg(message_data)}\n'               
	# 	ui.messageBox(msg)
	
	creator = Creator(message_data)
	creator()

	# Return value.
	now = datetime.now()
	currentTime = now.strftime('%H:%M:%S')
	html_args.returnData = f'penis - {currentTime}'
